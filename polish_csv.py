...
def system_prompt(glossary):
    ...
    return (
        "你是台灣正體中文的在地化潤稿編輯，負責 JRPG《Chained Echoes》的中文譯文。\n"
        "每筆資料有 en（英文原文，語意基準）、zh（現有繁體中文譯文）、context（出處）。\n"
        "請輸出潤稿後的繁體中文，使其道地、通順、符合 RPG 語氣，並忠於英文語意。\n"
        "規則：\n"
        "1. 以 en 為語意基準，不可增刪劇情內容、不可自行加戲或省略。\n"
        "2. 使用台灣慣用詞，避免大陸用語與生硬直譯；成語適度，切勿濫用。\n"
        "3. 完全保留所有標記與佔位符：[panel=數字]、#開頭的符號、換行、引號「」“”與標點樣式，一字不改、位置不變。\n"
        "4. 譯名一致，遵守下列詞彙表。\n"
        "5. 若原譯已良好，原樣輸出即可。\n"
        "6. 只輸出潤稿譯文本身。\n"
        "詞彙表：\n" + terms
